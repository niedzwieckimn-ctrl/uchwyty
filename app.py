...
def home():
    ...
    cur.execute("SELECT COUNT(*) AS n FROM orders WHERE status IN ('new','packed')")
...
def orders():
    ...
    if tab == "new":
        where_parts.append("status IN ('new', 'packed')")
    elif tab == "issued":
    ...
    visible_open_ids = sorted([r["id"] for r in rows if r["status"] in ("new", "packed")])
    if visible_open_ids:
        cur.execute("SELECT id FROM orders WHERE status IN ('new','packed') AND id<=? ORDER BY id", (visible_open_ids[-1],))
...
def order_create():
    ...
    # create order row first (temporary order_no)
    cur.execute("""
...
def order_view(order_id):
    ...
    if o["status"] in ("new", "packed"):
        cur.execute("SELECT id FROM orders WHERE status IN ('new','packed') AND id<=? ORDER BY id", (order_id,))
...
if "def supabase_pull_to_sqlite():" in s:
    print("✅ Funkcja pull już istnieje w app.py")
    raise SystemExit(0)
...
marker = '\n# =========================\n# RUN\n# =========================\n'
if marker not in s:
    raise SystemExit("❌ Nie znalazłem sekcji RUN w app.py")

s = s.replace(marker, "\n" + block + "\n" + marker)

# dodanie przycisku do template cloud
btn = '<form method="post" action="{{ url_for(\'cloud_supabase_pull\') }}" style="margin-top:10px;">'
if btn not in s and "Synchronizuj teraz" in s:
    s = s.replace(
        "Synchronizuj teraz</button>",
        "Synchronizuj teraz</button>\n        </form>\n"
        f'        {btn}\n          <button class="btn" type="submit">Pobierz z chmury</button>\n        </form>\n        <form'
    )

p.write_text(s, encoding="utf-8")
print("✅ app.py zaktualizowany automatycznie")
PY
