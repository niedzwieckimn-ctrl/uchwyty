...
# DB
# =========================
def _new_sqlite_write_lock():
    """The SQL wrapper can re-enter on the same request through order leases."""
    return threading.RLock()


_sqlite_write_lock = _new_sqlite_write_lock()
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{background:var(--bg)}body{font-family:Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:0;background:radial-gradient(circle at 85% -10%,#eaf0ff 0,transparent 28%),var(--bg);color:var(--ink);line-height:1.45}
    ...
    .nav a:before{width:25px;font-size:16px;opacity:.9}
...
<body>
  <style>
    .brand.admin-brand{height:92px;padding:0;justify-content:center;overflow:hidden;flex-shrink:0}.brand.admin-brand:before{display:none}.admin-brand img{width:170px;max-width:none;filter:invert(1);mix-blend-mode:screen}
  ...
  <div class="top">
    ...
    <div class="nav flex">
      ...
      <a class="nav-stock" href="{{ url_for('stock') }}">Stan magazynu</a>
      <a class="nav-remanent" href="{{ url_for('remanent_index') }}">Remanent</a>
      <a class="nav-import" href="{{ url_for('china') }}">Import</a>
      <a class="nav-scan" href="{{ url_for('order_scan') }}">Skan QR</a>
      ...
      <a class="nav-logout" href="{{ url_for('logout') }}">Wyloguj</a>
    </div>
