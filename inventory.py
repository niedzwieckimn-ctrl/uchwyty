...
def register_routes(context):
    ...
    @require_permission(
        "inventory.read",
        operation="inventory.product.read",
        audit_success=True,
        entity_type="product",
        entity_id_arg="product_id",
    )
    def api_product(product_id):
        ...
        if result.status == "DENIED":
            return jsonify(ok=False, error=result.safe_error_message), 403
        return jsonify(ok=False, error=result.safe_error_message or "Błąd operacji"), 500
